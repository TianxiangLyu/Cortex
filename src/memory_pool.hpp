...
namespace Cortex
{
    ...
    enum class MemoryAllocMode : int
    {
        Default = 0,
        Pool = 1,
        Stack = 2
    ...
    public:
        ...
        {
            if (getInstance().top_ != getInstance().bottom_ || getInstance().n_segment_ != 0 || getInstance().size_ != 0)
            {
                MemoryPool::dump();
            }
        ...
        static void initialize(const size_t _cap)
        {
            ...
            {
                std::cerr << "CX_ERROR: malloc failed. (function: " << __func__
        ...
        static void alloc(const size_t _size, int &_id_mpool, void *ptr_data, void *&ret, MemoryAllocMode mode = MemoryAllocMode::Pool)
        {
            ...
            if (!flag_break)
            {
                ...
                if (flag_realloc && inParallelRegion())
                    flag_use_emerg_bufs = true;
        ...
        static void freeMem(const int id_seg)
        {
            if (id_seg >= 0)
            {
                ...
                if (id_seg < N_SEGMENT_LIMIT)
                {
                    if (getInstance().cap_per_seg_[id_seg] <= 0)
                        return;
                    ...
                    if ((size_t)id_seg == getInstance().n_segment_ - 1)
                    {
                        ...
                        {
                            if (getInstance().used_per_seg_[i] == true)
                                break;
