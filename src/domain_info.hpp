...
namespace Cortex
{
    ...
    public:
        ...
        void initialize_group(const F64 coef_ema)
        {
            DeleteArray(n_smp_array_);
            DeleteArray(n_smp_disp_array_);
            DeleteArray(pos_domain_);
            DeleteArray(pos_domain_temp_);
            S32 n_proc;
            MPI_Group_size(comm_info_.getGroup(), &n_proc);
            const S32 my_rank = comm_info_.getRank();
            n_smp_array_ = new S32[n_proc];
            n_smp_disp_array_ = new S32[n_proc + 1];
#ifdef CORTEX_MPI_PARALLEL
        ...
        void setCommInfo(const CommInfo &c,
                         const F64 coef_ema = CORTEX_DFLT_VAL_COEF_EMA)
        {
            comm_info_ = c;
            if (comm_info_.isNotCommNull())
        ...
        void initialize(const F32 coef_ema = CORTEX_DFLT_VAL_COEF_EMA)
        {
            if (coef_ema < 0.0 || coef_ema > 1.0)
            {
                CORTEX_PRINT_ERROR("The smoothing factor of an exponential moving average is must between 0 and 1.");
        ...
        void setNumberOfDomainMultiDimension(const S32 nx, const S32 ny, const S32 nz = 1)
        {
            ...
            if (n_proc != nx * ny * nz)
            {
                CORTEX_PRINT_ERROR("devided number of domains is not consistent with total processe number");
                Abort(-1);
            }
            n_domain_[0] = nx;
            n_domain_[1] = ny;
        ...
        void decomposeDomain()
        {
            ...
            */
            ...
            if (myrank == 0 || decompose_in_all_processes)
            {
                ...
                F64 wtmp = GetWtime();
#if defined(CORTEX_USE_SAMPLE_SORT)
                SampleSortLib::samplesort_bodies(
                    pos_sample_tot_.getPointer(),
       ...
       // --- process first ------------------------
                ...
                else
                {
                    for (S32 i = 0; i < nproc; i++)
                    {
                        pos_domain_[i].low_ = (F64)coef_ema_ * pos_domain_temp_[i].low_ + (F64)(1. - coef_ema_) * pos_domain_[i].low_;
                        pos_domain_[i].high_ = (F64)coef_ema_ * pos_domain_temp_[i].high_ + (F64)(1. - coef_ema_) * pos_domain_[i].high_;
                    }
                }
                // ------------------------------------------
                delete[] istart;
        ...
        */
                              ...
                              const F64vec &high)
        {
            for (S32 k = 0; k < DIMENSION; k++)
            {
                if (low[k] > high[k])
                {
                    CORTEX_PRINT_ERROR("The coodinate of the root domain is inconsistent.");
                              ...
                              const S32 shift = 1)
        {
            ...
            auto rank = my_rank;
            auto x_tmp = (x < coord_sep[rank]) ? (coord_sep[rank] - x < x + len_peri - coord_sep[rank + 1] ? x : x + len_peri) : (x - coord_sep[rank + 1] < coord_sep[rank] - (x - len_peri) ? x : x - len_peri);
                                     ...
                                     bool print = false)
        {
            ...
            RandomSampling(coord, coord_smp, [](F64 &src, F64 &dst)
                           { dst = src; });
        ...
        void decomposeDomainMultiStep2(const bool flag_use_old_domain = false)
        {
            ...
            gatherSampleRandomly(pos_sample_loc_, x_smp_sub_in_slab, n_smp_sub_x, comm_info_sub_[0], [](F64vec &src, F64 &dst)
                                 { dst = src.x; });
            ...
            if (comm_info_sub_[0].getRank() == 0)
            {
                ...
                exchangeSample(x_smp_sub_in_slab, x_partition_tmp, n_proc_x, my_rank_x, full_len_root_domain[0], periodic_axis_[0], comm_info_1d_[0], [](const F64 &src) -> F64
                               { return src; });
            ...
            exchangeSample(pos_sample_loc_, x_partition, n_proc_x, my_rank_x, full_len_root_domain[0], periodic_axis_[0], comm_info_1d_[0], [](const F64vec &src) -> F64
                           { return src.x; });
#ifdef CX_DEBUG_DD
            ...
            gatherSampleRandomly(pos_sample_loc_, y_smp_sub_in_rod, n_smp_sub_y, comm_info_1d_[2], [](F64vec &src, F64 &dst)
                                 { dst = src.y; });
            ...
            if (my_rank_z == 0)
            {
                ...
                exchangeSample(y_smp_sub_in_rod, y_partition_tmp, n_proc_y, my_rank_y, full_len_root_domain[1], periodic_axis_[1], comm_info_1d_[1], [](const F64 &src) -> F64
                               { return src; });
            ...
            exchangeSample(pos_sample_loc_, y_partition, n_proc_y, my_rank_y, full_len_root_domain[1], periodic_axis_[1], comm_info_1d_[1], [](const F64vec &src) -> F64
                           { return src.y; });
#ifdef CX_DEBUG_DD
            for (S32 i = 0; i < pos_sample_loc_.size(); i++)
            {
            ...
            exchangeSample(z_smp_sub, z_partition_tmp, n_proc_z, my_rank_z, full_len_root_domain[2], periodic_axis_[2], comm_info_1d_[2], [](const F64 &src) -> F64
                           { return src; });
