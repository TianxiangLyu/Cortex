...
namespace Cortex
{
    ...
    //////////////////
    // between points
    CORTEX_INLINE F64 GetDistanceMin1DImpl(const F64 a,
                                           const F64 b)
    {
        return std::fabs(a - b);
    }

    CORTEX_INLINE F64 GetDistanceMin1DPeriImpl(const F64 a,
                                               const F64 b,
                                               const F64 len)
    {
    ...
    template <int bit>
    CORTEX_INLINE F64 GetDistanceMin1D(const F64 a,
                                       const F64 b,
                                       const F64 len)
    {
        Abort();
    ...
    template <>
    CORTEX_INLINE F64 GetDistanceMin1D<0>(const F64 a,
                                          const F64 b,
                                          const F64 len)
    {
    ...
    template <>
    CORTEX_INLINE F64 GetDistanceMin1D<1>(const F64 a,
                                          const F64 b,
                                          const F64 len)
    {
    ...
    //////////////////
    // between lines
    CORTEX_INLINE F64 GetDistanceMin1DImpl(const F64 al,
                                           const F64 ah,
                                           const F64 bl,
                                           const F64 bh)
    {
    ...
    CORTEX_INLINE F64 GetDistanceMin1DPeriImpl(const F64 al,
                                               const F64 ah,
                                               const F64 bl,
                                               const F64 bh,
                                               const F64 len)
    ...
    template <int bit>
    CORTEX_INLINE F64 GetDistanceMin1D(const F64 al,
                                       const F64 ah,
                                       const F64 bl,
                                       const F64 bh,
                                       const F64 len)
    ...
    template <>
    CORTEX_INLINE F64 GetDistanceMin1D<0>(const F64 al,
                                          const F64 ah,
                                          const F64 bl,
                                          const F64 bh,
                                          const F64 len)
    ...
    template <>
    CORTEX_INLINE F64 GetDistanceMin1D<1>(const F64 al,
                                          const F64 ah,
                                          const F64 bl,
                                          const F64 bh,
                                          const F64 len)
    ...
    //////////////////
    // between F64vec
    CORTEX_INLINE F64 GetDistanceMinSq(const F64vec &pos0,
                                       const F64vec &pos1)
    {
    ...
    //////////////////
    // between F64ort
    CORTEX_INLINE F64 GetDistanceMinSq(const F64ort &pos0,
                                       const F64ort &pos1)
    {
...
#if __cplusplus >= CX_CPLUSPLUS_17
    template <typename...>
    constexpr bool FALSE_V = false;
    template <typename T0, typename T1>
    CORTEX_INLINE bool IsOverlapped(const T0 &pos0,
                                    const T1 &pos1)
    {
        if constexpr ((std::is_same_v<T0, F64vec> && std::is_same_v<T1, F64vec>) || (std::is_same_v<T0, F64ort> && std::is_same_v<T1, F64ort>) || (std::is_same_v<T0, F64vec> && std::is_same_v<T1, F64ort>) || (std::is_same_v<T0, F64ort> && std::is_same_v<T1, F64vec>))
        {
            // asm("#IF TRUE");
...
#else
    CORTEX_INLINE bool IsOverlapped(const F64ort &pos0,
                                    const F64ort &pos1)
    {
        return GetDistanceMinSq(pos0, pos1) <= 0.0;
    }
    CORTEX_INLINE bool IsOverlapped(const F64vec &pos0,
                                    const F64vec &pos1)
    {
        return GetDistanceMinSq(pos0, pos1) <= 0.0;
    }
    ...
    static inline void CalcNumberAndShiftOfImageDomain(ReallocatableArray<F64vec> &shift_image_domain,
                                                       ...
                                                       const bool clear = true)
    {
        ...
        for (S32 lev = 1;; lev++)
        {
            ...
            S32 lev_x = 0;
            S32 lev_y = 0;
            if (periodic_axis[0] == true)
                lev_x = lev;
            if (periodic_axis[1] == true)
                lev_y = lev;
        ...
        for (S32 lev = 1;; lev++)
        {
            ...
            for (S32 ix = -lev_x; ix <= lev_x; ix++)
            {
                for (S32 iy = -lev_y; iy <= lev_y; iy++)
                {
                    ...
                    {
                        if ((std::abs(ix) != lev_x || periodic_axis[0] == false) && (std::abs(iy) != lev_y || periodic_axis[1] == false) && (std::abs(iz) != lev_z || periodic_axis[2] == false))
                            continue;
    ...
    static CORTEX_INLINE S32vec GetRankVec(const S32 n_domain[DIMENSION_LIMIT],
                                           const S32 rank_glb,
                                           const S32 dim)
    {
        S32vec rank_1d;
    ...
    static CORTEX_INLINE S32 GetRankGlb(const S32vec rank_vec,
                                        const S32 n_domain[DIMENSION_LIMIT])
    {
#if defined(CORTEX_TWO_DIMENSION)
    ...
    static CORTEX_INLINE S32 GetRank1d(const S32 rank_glb,
                                       const S32 n_domain[DIMENSION_LIMIT],
                                       const S32 cid)
    {
        S32 ret = -1;
    ...
    static CORTEX_INLINE void CalcRank1D(S32 &rank_1d, S32 &dnp_1d, F64 &dis_domain_1d, const F64vec &pos, const S32 rank_glb_org, const S32 n_domain[DIMENSION_LIMIT], const F64ort domain[], const F64 len_peri, const bool pa, const int cid)
    {
