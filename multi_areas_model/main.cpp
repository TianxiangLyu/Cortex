...
#pragma omp for
            ...
            for (CX::S32 i = 0; i < population.getNumLocal(); ++i)
                population[i].input_syn_ex_ += d(eng) * weight;
        }
    }
}
template <class Tpopulation>
...
int main(int argc, char *argv[])
{
    ...
    {
        std::cout << "===========================================" << std::endl
                  << "Cortex Multi Areas Model " << std::endl
                  << "===========================================" << std::endl
    ...
    for (CX::F64 time = 0; time < presimtime; time += dt, step++)
    {
        ...
        L1e.NeuralDynamics(iaf_psc_exp<>::CalcDynamics(time, dt));
        L1i.NeuralDynamics(iaf_psc_exp<>::CalcDynamics(time, dt));
    }
    ...
    for (CX::F64 time = presimtime; time < presimtime + simtime; time += dt, step++)
    {
        ...
        /* if (CX::Comm::getRank() == 0)
        {
            ...
            std::cout<<"V_m_ "<<L1e[0].V_m_<<" r_ref_ "<<L1e[0].r_ref_ <<" spike "<<L1e[0].spike<<" spike num"<<L1e[0].spikeNum<<std::endl;
        } */
    }
    CX::Comm::barrier();
