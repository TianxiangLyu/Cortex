...
namespace Cortex
{
    ...
    private:
        ...
        CORTEX_INLINE S32 searchWhichDomainParticleGoTo(const F64vec &pos,
    ...
    public:
                              ...
                              const char *const open_format)
        {
            ...
            { // Read from single file
                ...
                if (fp == NULL)
                {
                    CORTEX_PRINT_ERROR("can not open input file ");
                    std::cerr << "filename: " << filename << std::endl;
                    Abort(-1);
                }
                S32 n_ptcl_ = (header->*pFuncHead)(fp);
                if (n_ptcl_ < 0)
                { // User does NOT return # of ptcl
...
// Read remaining data to buffer and send them to appropriate process.
                               ...
                               const char *const open_format) const
        {
            if (format == NULL)
            {
                ...
                if (comm_info_.getRank() == 0)
                {
                    ...
                    if (fp == NULL)
                    {
                        CORTEX_PRINT_ERROR("can not open output file");
        ...
        S64 getNumberOfParticleGlobal() const
        {
            ...
            return Comm::getSum(n_ptcl_loc); // Cortex using global communication
...
#ifdef CORTEX_MPI_PARALLEL
            RandomSampling(
                ptcl_, pos_sample, [](const Tptcl &src, F64vec &dst)
                { dst = src.getPos(); },
                0, offset_pos_sample);
            return;
#endif
...
// modified to limit # of sample particles by M.I. 
                              ...
                              const bool flag_serialize = false)
        {
            ...
            {
                CORTEX_PRINT_ERROR("serialization is not yet supported.");
                Abort(-1);
            }
            if (dinfo.getBoundaryCondition() == BOUNDARY_CONDITION_NULL)
                return;
            if (comm_info_.isCommNull())
                return;
            ...
            CX_OMP_PARALLEL
            {
                ...
                for (S32 ip = ip_head; ip < ip_end; ip++)
                {
                    ...
                    dnp_max_tmp = ApplyEach([](const F64 &l, const F64 &r)
                                            { return std::max(l, r); },
                                            dnp_max_tmp, dnp);
                }
                ...
                CX_OMP_SINGLE
                {
                    ...
                    {
                        dnp_max_loc = ApplyEach([](const S32 &l, const S32 &r)
                                                { return std::max(l, r); },
                                                dnp_max_loc, dnp_max_loc_per_thread[i]);
                    }
                    n_disp_send[0] = 0;
                    for (S32 i = 0; i < n_proc; i++)
                    {
      ...
      // non-parallel version
            ...
            for (auto ip = n_loc - 1; ip >= 0; ip--)
            {
                ...
                {
                    CORTEX_PRINT_ERROR("A particle is out of root domain");
                ...
                iloc--;
                dnp_max_loc = ApplyEach([](const F64 &l, const F64 &r)
                                        { return std::max(l, r); },
                                        dnp_max_loc, dnp);
                n_send[srank]++;
            }
        ...
        {
            F64 sum_loc, sum_max;
            sum_loc = (double)(ptcl_.getMemSize() + idx_remove_ptcl_.getMemSize()) * 1e-9;
