...
struct iaf_psc_exp_default
{
    ...
    constexpr static const CX::F64 V_reset = -70.0 - E_L_; // mV, rel to E_L
    constexpr static const CX::F64 V_th = -55.0 - E_L_;    // mV, rel to E_L
...
public:
    ...
    struct Neuron
    {
        ...
        CX::S32 r_ref_; //!< Absolute refractory counter (no membrane potential propagation)
    ...
    private:
        ...
        std::normal_distribution<CX::F64> d;
    public:
        CalcDynamics(const CX::F64 _time,
            ...
            : time(_time),
              ...
              P22_(-h / Tmodel::Tau_),
              P20_(Tmodel::Tau_ / Tmodel::C_ * ( 1.0 - P22_ )),
              rd(),
              eng(rd()),
              d(0, 1.0){};
...
#pragma omp parallel for
            ...
            for (CX::S32 i = 0; i < neuron.getNumberOfParticleLocal(); ++i)
            {
                if (neuron[i].r_ == 0)
                {
                    neuron[i].V_m_ = neuron[i].V_m_ * P22_ 
                                   + neuron[i].i_syn_ex_ * P21ex_
                                   + neuron[i].i_syn_in_ * P21in_
                                   + (neuron[i].I_e_ + neuron[i].i_0_ ) * P20_;
                }
                else
                {
                    neuron[i].r_--;
                ...
                if ((Tmodel::delta_ < 1e-10 && neuron[i].V_m_ >= Tmodel::delta_) 
                || (Tmodel::delta_ > 1e-10 && d(eng) < phi(neuron[i].V_m_) * h * 1e-3))
                {
                    neuron[i].GetSpike(time);
                    neuron[i].V_m_ = Tmodel::V_reset;
                    neuron[i].r_ref_ = Tmodel::t_ref;
                }
