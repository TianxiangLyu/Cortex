...
namespace Cortex
{
    ...
    public:
        ...
        Orthotope2(const Vector2<T> &center, const T length) : low_(center - (Vector2<T>)(length)), high_(center + (Vector2<T>)(length))
        {
        }
        ...
        void setNull()
        {
            low_.x = std::numeric_limits<float>::max() / 128;
            low_.y = std::numeric_limits<float>::max() / 128;
            high_.x = std::numeric_limits<float>::max() / 128;
            high_.y = std::numeric_limits<float>::max() / 128;
        }

        bool isNull()
        {
            return (low_.x > std::numeric_limits<float>::max() / 256 &&
                    low_.y > std::numeric_limits<float>::max() / 256 &&
                    high_.x > std::numeric_limits<float>::max() / 256 &&
                    high_.y > std::numeric_limits<float>::max() / 256);
        }
...
*/
        ...
        {
            return (pos.x < low_.x) || (high_.x <= pos.x) || (pos.y < low_.y) || (high_.y <= pos.y);
        }
        ...
        unsigned int notContained(const Orthotope2 &a) const
        {
            ...
            return (a_high.x < b_low.x) || (b_high.x < a_low.x) || (a_high.y < b_low.y) || (b_high.y < a_low.y);
        }
        ...
        unsigned int notOverlapped(const Orthotope2 &a) const
        {
            return (a.high_.x < low_.x) || (high_.x < a.low_.x) || (a.high_.y < low_.y) || (high_.y < a.low_.y);
        }
